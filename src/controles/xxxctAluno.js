...
module.exports = {
    ...
    async lista(req, res) {
        try {
            ...
            //-----------------------------------
            var indA = -1;
            alunos.forEach(async(elementAl, indexAl, arrayAl) => {
                indA = indA + 1;
                ...
                try {
                    ...
                    tarefas.forEach(async(elementTr, indexTr, arrayTr) => {
                        indT = indT + 1;
                        let tarefaId = tarefas[indexTr].id;
                        const tarefasAluno = await TarefaAluno.findAll({ raw: true, where: {idAluno: alunoId, idTarefa: tarefaId}});
                        console.log(indA,indT);
                        if (tarefasAluno.length > 0) {
                            //console.log("#######################################################");
                            //listatarefas.push(alunoId, tarefaId, tarefasAluno[0].dataEntrega, tarefasAluno[0].conceito);
                            listatarefas[indA][indT] = [ alunoId, tarefaId, tarefasAluno[0].dataEntrega, tarefasAluno[0].conceito];
                            //listatarefas.push("ENTREGOU");
                            console.log(listatarefas[indA][indT]);
                        } else {
                            console.log(".......................................................");
                        ...
                        //if (tarefasAluno.length > 0) {
                        //    var counttar = listatarefas.push(alunoNome);
                        //    console.log(listatarefas);
                        //     
                        //    console.log(alunoNome);
                        //    //listatarefas.push(tarefasAluno[0].conceito);
                        //    console.log(counttar, "  -----------------------------------------------------------------------------")
                        //    //console.log("            Tarefa ",tarefaId, "pedida em ", tarefaPedida, "@", alunoId, " Nome do aluno ", alunoNome , " - NÃO TEM tarefa *")
                        //} else {
                        //    //console.log("############Tarefa ",tarefaId, "pedida em ", tarefaPedida,  " Nome do aluno ", alunos[indexAl].nome, " - Data da tarefa ", tarefas[indexTr].dataEntrega);
                        //} 
                    });
                    console.table(listatarefas);
